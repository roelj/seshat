...
class WebServer:
    ...
    def __init__ (self, address="127.0.0.1", port=8080):
        ...
        self.url_map = Map([
            ...
            ## OCI REGISTRY v2
            ## ----------------------------------------------------------------
            R("/v2/",                                                            self.api_oci_registry_version),
            R("/v2/_catalog",                                                    self.api_oci_registry_catalog),
            R("/v2/<path:name>/tags/list",                                       self.api_oci_registry_tags),
            R("/v2/<path:name>/manifests/<reference>",                           self.api_oci_registry_manifest),
            R("/v2/<path:name>/blobs/<digest>",                                  self.api_oci_registry_blob),
            R("/v2/<path:name>/blobs/uploads/",                                  self.api_oci_registry_blob_upload_start),
            R("/v2/<path:name>/blobs/uploads/<upload_uuid>",                     self.api_oci_registry_blob_upload),
            R("/oci/token",                                                      self.api_oci_registry_token),
        ])
    ...
    def token_from_request (self, request):
        ...
        try:
            ...
            if isinstance(token_string, str):
                ...
                elif token_string.startswith("Bearer "):
    ...
    def ui_home (self, request):
        ...
        try:
            ...
            for rec in records:
                authors = []
                if rec.get("embargo_type") == "article":
                    authors.append({"full_name": "Undisclosed due to embargo"})
                else:
    ...
    def api_v3_reviewers (self, request):
        ...
        reviewers = self.db.reviewer_accounts ()
        reviewers += self.db.institutional_reviewer_accounts ()

        return self.default_list_response (reviewers, formatter.format_account_record)
    ...
    def __oci_registry_stream_to_file (self, request, destination):
        bytes_to_read = request.content_length
        input_stream = request.stream
    ...
    def __registry_require_auth (self, request):
        """Return None when the request is authorised, or a 401 otherwise."""
        token = self.token_from_request (request)
    ...
    def api_oci_registry_token (self, request):
        ...
        token = config.oci_anonymous_token_value
    ...
    def api_oci_registry_catalog (self, request):
        ...
        manifests_root = os.path.join (config.oci_registry_storage, "manifests")
        repositories = []
        if os.path.isdir (manifests_root):
            for dirpath, _, filenames in os.walk (manifests_root):
                ...
                if any (not file.endswith (".ct") for file in filenames):
    ...
    def api_oci_registry_tags (self, request, name):
        ...
        if os.path.isdir (manifest_dir):
            for entry in os.listdir (manifest_dir):
                ...
                if not (entry.endswith (".ct") or entry.startswith ("sha256:")):
                    tags.append (entry)
    ...
    def api_oci_registry_manifest (self, request, name, reference):
        ...
        if request.method in ("GET", "HEAD"):
            ...
            digest = self.__oci_registry_compute_sha256 (manifest_path)
        ...
        if request.method == "DELETE":
            ...
            target_digest = self.__oci_registry_compute_sha256 (manifest_path)
            self.locks.lock (locks.LockTypes.OCI_REGISTRY)
            try:
                for entry in os.listdir (manifest_dir):
                    ...
                    if self.__oci_registry_compute_sha256 (entry_path) != target_digest:
                        continue
